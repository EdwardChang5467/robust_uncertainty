...
cache_path = "YOUR_CACHE_DATA_SAVE_PATH"
...
def name_checker(model_name,cache_path,num_of_gen):
    ...
    for idx,fake_name in enumerate(data):
        message = f'''
[Instruction]:
When providing an answer of the question based on the actual situation, please provide a reason and a judgment of "yes" or "no". Don't correct the name in the question.

[Question]:
Does {fake_name} a real person?
...
'''
    ...
    with open(f"{cache_path}/{model_name}_name_checker_reply_sample7.json","w",encoding="utf-8") as file:
        json.dump(model_reply, file, ensure_ascii=False, indent=4)
    with open(f"{cache_path}/{model_name}_name_checker_answer_sample7.json","w",encoding="utf-8") as file:
        json.dump(model_answer, file, ensure_ascii=False, indent=4)
    with open(f"{result_path}/{model_name}_cleaned_fake_names_sample7.json","w",encoding="utf-8") as file:
        json.dump(final_fake_name_list, file, ensure_ascii=False, indent=4)
    

if __name__ == "__main__":
    name_checker("yi-lightning",cache_path,7)  
